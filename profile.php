...
<body onload="checkActiveMessage('<?php echo $_SESSION['auth']['uid']; ?>'); loadRecommend('<?php echo $_SERVER['REQUEST_URI']; ?>');"> <!--  -->
		...
		<nav class="navbar">
			<ul>
				...
				<?php if($logged) { ?>
					...
					<?php } else { ?>
						<li><a href="<?php echo $isOwner->username; ?>" id="profile" class="nav-link">myProfile</a></li>
					<?php } ?>
	...
	<div class="hidden-menu"> 
		<?php if($logged) { ?>
			<li><i class="fa fa-user"></i> <a href="<?php echo $isOwner->username; ?>" id="profile">myProfile</a></li>
			<li><i class="fa fa-cog"></i> <a href="javascript:openSettings()" id="settings">Settings</a></li>
			<li><i class="fa fa-pencil"></i> <a href="javascript:editPopup()">Edit</a></li>
	...
	<div class="container-profile">
		...
		<div class="username">
			...
			<?php if($logged && $isOwner->id == $row['id']) { ?>
				<span class="fa fa-pencil edit-profile" title="Edit Profile" onclick="editPopup()"></span>
	...
	<div class="container-item">
		<div>
			...
			<?php } elseif($logged && $isOwner->id != $row['id']) {
			  ...
			  <?php } else { ?>
						<button type="button" id="follow" class="btn" data-follow_user="" onclick="followUser(this, <?php echo $row['id']; ?>)">Follow</button>
			  <?php } ?>
					<button type="button" id="message" class="btn" onclick="openMessaging('<?php echo $row['id']; ?>');">Message</button>
			  <?php } else { ?>
					<button type="button" class="btn followUser" onclick="alert('Please login or create account!')">Follow</button>
		...
		<div class="owner-posts">
			<?php if($stats['total_posts'] > 0) { 
					foreach($mediaposts as $posts): ?>
						<li class="post-resource" onclick="zoomInImage('<?php echo $posts->urlpath; ?>','<?php echo $posts->media_id; ?>')"><img src="images/wallposts/<?php echo $posts->urlpath; ?>"></li>
			<?php 	endforeach; 
				  } else { ?>
